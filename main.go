...
func index(db *sql.DB, fs TemplateFS) http.HandlerFunc {
	...
	return hx_template_handler(fs, files, template.FuncMap{
...
func todos(db *sql.DB, fs TemplateFS) http.HandlerFunc {
	...
	return hx_template_handler(fs, files, template.FuncMap{
		...
		"toggleall": func() (changed bool, err error) {
			res, err := db.Exec("UPDATE todos SET done = NOT (SELECT COALESCE(MIN(done),0) from todo)")
		...
		"delete": func(id string) (Unit, error) {
			idn, err := strconv.ParseInt(id, 10, 64)
			if err != nil {
				return Unit{}, err
			}
			...
			if affected, _ := res.RowsAffected(); affected != 1 {
				return Unit{}, fmt.Errorf("invalid todo id")
			}
			if err != nil {
				return Unit{}, err
			}
		...
		"todos": func() ([]Todo, error) {
			return RowsScanner(scanTodo)(db.Query(
				`WITH f(filter) AS (SELECT value FROM kv WHERE key = ?)
				SELECT id, done, label
				FROM todos
				JOIN f
				WHERE done = 1 AND f.filter in ('all','completed')
				OR done = 0 AND f.filter in ('all','active')
				ORDER BY id DESC;`, FILTER_KEY))
		},
...
func UnitFn() Unit { return Unit{} }

func hx_template_handler(fs TemplateFS, files []string, funcs template.FuncMap) http.HandlerFunc {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// Construct the routeId, re ~= (hx-)?{httpMethod}{-urlParams}
		// Examples: get, hx-get, post-id, hx-post-id

		keys := maps.Keys(params)
		sort.Strings(keys)
		routeparts := append([]string{"hx", strings.ToLower(r.Method)}, keys...)
		if r.Header.Get("HX-Request") != "true" {
			routeparts = routeparts[1:]
		}
		routeId := strings.Join(routeparts, "-")

		log.Printf("Executing route: %s : %s", r.URL.Path, routeId)

		var err error
		if routeId == "get" {
			err = tmpl.Execute(w, params)
		} else if t := tmpl.Lookup(routeId); t != nil {
